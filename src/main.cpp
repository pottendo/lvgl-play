 ...
 *   STATIC FUNCTIONS
 **********************/
static void mouse_action(struct _lv_indev_drv_t *drv, uint8_t ev)
{
    if (ev == LV_EVENT_PRESSED)
    {
        lv_indev_data_t d;
        mouse_read(drv, &d);
        select_start(d.point);
    }
    ...
    else if (ev == LV_EVENT_PRESSING)
    {
        lv_indev_data_t d;
        mouse_read(drv, &d);
        select_update(d.point);
    }
}
...
static void hal_init(void)
{
   ...
   * Use the 'mouse' driver which reads the PC's mouse*/
    ...
    indev_drv.feedback_cb = mouse_action;
